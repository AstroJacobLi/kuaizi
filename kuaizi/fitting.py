...
def _fitting_wavelet(data, coord, pixel_scale=HSC_pixel_scale, zp=HSC_zeropoint, prefix='mockgal', index=0):
    '''
    ...
    box_flag = np.unique(segmap[starlet_extent[0]:starlet_extent[1], starlet_extent[2]:starlet_extent[3]]) - 1
    ...
    box_flag = np.delete(box_flag, np.where(box_flag == cen_indx_big)[0]) # dont include the central galaxy
    ...
    if len(obj_cat_big) > 0:
        if len(star_cat) > 0:
            ...
            big_cat = obj_cat_big[np.setdiff1d(np.arange(len(obj_cat_big)), tempid[np.where(sep2d < 2 * u.arcsec)])]
        else:
            big_cat = obj_cat_big
    ...
    fig = kz.display.display_scarlet_sources(
        ...
        scale_bar_length=10,
        add_text=f'{prefix}-{index}')
    plt.savefig(f'./Figures/{prefix}-{index:04d}-src-wavelet.png', bbox_inches='tight')
    ...
    try:
        ...
        fig = kz.display.display_scarlet_model(
                    ...
                    show_mark=False,
                    scale_bar=False)
        plt.savefig(f'./Figures/{prefix}-{index:04d}-fitting-wavelet.png', bbox_inches='tight')
        fig = kz.display.display_scarlet_model(
                    blend,
                    show_ind=sed_ind,
                    zoomin_size=50, 
                    ...
                    show_mark=False,
                    scale_bar=False)
        plt.savefig(f'./Figures/{prefix}-{index:04d}-zoomin-wavelet.png', bbox_inches='tight')
        ...
        fig = kz.display.display_scarlet_model(
                    ...
                    show_mark=False,
                    scale_bar=False)
        plt.savefig(f'./Figures/{prefix}-{index:04d}-zoomin-mask-wavelet.png', bbox_inches='tight')
...
def fitting_wavelet_observation(lsbg, hsc_dr, cutout_halfsize=1.0, prefix='LSBG', pixel_scale=HSC_pixel_scale, zp=HSC_zeropoint):
    clear_output()
    from kuaizi.utils import padding_PSF
    kz.utils.set_env(project='HSC', name='HSC_LSBG')
    # kz.utils.set_matplotlib(usetex=False, fontsize=15)
    ...
    blend = _fitting_wavelet(data, lsbg_coord, prefix=prefix, index=index, pixel_scale=pixel_scale, zp=zp)
    return blend

def fitting_wavelet_mockgal(index=0, prefix='MockLSBG', pixel_scale=HSC_pixel_scale, zp=HSC_zeropoint):
    clear_output()
    kz.utils.set_env(project='HSC', name='HSC_LSBG')
    index = index
