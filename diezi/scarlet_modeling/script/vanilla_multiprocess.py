...
def run_scarlet_vanilla(index, cat, DATADIR, OUTPUT_DIR, PREFIX, starlet_thresh=0.5, global_logger=None, fail_logger=None):
    blend = fitting_obs_tigress(
        ...
        prefix=PREFIX.lower(),
        figure_dir=os.path.join(OUTPUT_DIR, f'Figure/{PREFIX.lower()}/'),
        model_dir=os.path.join(OUTPUT_DIR, f'Model/{PREFIX.lower()}/'),
        log_dir=os.path.join(OUTPUT_DIR, f'log/{PREFIX.lower()}/'),
        show_figure=False,
...
def multiprocess_fitting(DATADIR, OUTPUT_DIR, PREFIX, njobs, cat_dir, ind_list=None, low=0, high=None, suffix='', starlet_thresh=0.5):
    ...
    pool.map(partial(run_scarlet_vanilla, DATADIR=DATADIR, OUTPUT_DIR=OUTPUT_DIR, PREFIX=PREFIX,
