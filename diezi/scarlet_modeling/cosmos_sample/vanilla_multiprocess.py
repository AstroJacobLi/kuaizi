...
def run_scarlet_vanilla(index, cat, starlet_thresh=0.5, global_logger=None, fail_logger=None):
    blend = fitting_vanilla_obs_tigress(
        ...
        prefix='cosmos',
        figure_dir='/tigress/jiaxuanl/Data/HSC/LSBG/Figure/COSMOS/',
        model_dir='/tigress/jiaxuanl/Data/HSC/LSBG/Model/COSMOS/',
        log_dir='/tigress/jiaxuanl/Data/HSC/LSBG/log/COSMOS/',
        show_figure=False,
...
def multiprocess_fitting(njobs, cat_dir, ind_list=None, low=0, high=None, suffix='', starlet_thresh=0.5):
    ...
    fail_logger = kz.utils.set_logger(
        logger_name='cosmos_fail' + suffix, file_name='cosmos_fail' + suffix, level='ERROR')
    global_logger = kz.utils.set_logger(
        logger_name='cosmos_sample' + suffix, file_name='cosmos_log' + suffix, level='INFO')
